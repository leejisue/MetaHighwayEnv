...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None, render_mode: str | None = None) -> None:
        ...
        # Fast flag: skip _automatic_rendering() entirely in headless mode
        # to avoid per-sub-step function-call overhead during training.
        self._headless = render_mode is None
    ...
    def _simulate(self, action: Action | None = None) -> None:
        ...
        for frame in range(frames):
            ...
            # has been launched.  Skipped entirely in headless mode (training)
            # to avoid per-sub-step function-call overhead.
            if not self._headless and frame < frames - 1:
