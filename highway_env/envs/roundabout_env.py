...
class RoundaboutEnv(CausalInterventionMixin, AbstractEnv):
    """
    A roundabout driving environment.
    ...
    #  incoming_vehicle_destination  EPISODE-LEVEL  dest index (requires reset)
    ...
    def _rewards(self, action: int) -> dict[str, float]:
        ...
        if isinstance(action, (int, np.integer)):
            ...
            # For continuous actions, no explicit lane change reward
            lane_change = 0
        return {
            "collision_reward": self.vehicle.crashed,
            "high_speed_reward": np.clip(
                utils.lmap(self.vehicle.speed, [0, self.vehicle.MAX_SPEED], [0, 1]),
                0, 1,
            ),
